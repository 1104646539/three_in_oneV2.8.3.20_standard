...
public class SqlInfoBuilder {
    ...
    public static SqlInfo buildInsertSql(DbUtils db, List<?> entities) throws DbException {

        if(entities == null || entities.size() == 0) return null;
        List<KeyValue> keyValueList = entity2KeyValueList(db, entities.get(0));
        if (keyValueList.size() == 0) return null;
        ...
        sqlBuffer.append("INSERT INTO ");
        sqlBuffer.append(TableUtils.getTableName(entities.get(0).getClass()));
        sqlBuffer.append(" (");
        for (KeyValue kv : keyValueList) {
            sqlBuffer.append(kv.key).append(",");
        }
        ...
        for (int i = 1; i < entities.size(); i++) {
            keyValueList = entity2KeyValueList(db, entities.get(i));
    ...
    public static SqlInfo buildCreateTableSqlInfo(DbUtils db, Class<?> entityType) throws DbException {
        ...
        for (Column column : columns) {
            if (column instanceof Finder) { // || column.getColumnField().getType() == boolean.class
                continue;
            }
