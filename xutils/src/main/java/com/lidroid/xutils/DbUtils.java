...
public class DbUtils {
    ...
    public int save(Object entity, boolean b) throws DbException {
        int lastInsertId = 0;
        try {
            beginTransaction();
            createTableIfNotExist(entity.getClass());
            execNonQuery(SqlInfoBuilder.buildInsertSqlInfo(this, entity));
            Cursor cursor = execQuery("select max (id) from " + TableUtils.getTableName(entity.getClass()));
            if (cursor != null) {
                cursor.moveToNext();
    ...
    public void saveAll(List<?> entities, boolean b) throws DbException {
        try {
            createTableIfNotExist(entities.get(0).getClass());
            int extra = entities.size() % 100 > 0 ? 1 : 0;
            for (int i = 0; i < entities.size() / 100 + extra; i++) {
                List<?> list = null;
                if (i != entities.size() / 100 + extra - 1) {//前面的数据
                    list = entities.subList(i * 100, (i + 1) * 100);
                    execNonQuery(SqlInfoBuilder.buildInsertSql(this, list));
                } else {//最后一组数据
                    list = entities.subList(i * 100, entities.size());
                    execNonQuery(SqlInfoBuilder.buildInsertSql(this, list));
                }
            }
        } finally {
